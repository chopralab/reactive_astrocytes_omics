...
library(plyr)
library(tidyverse)
protein_groups <- read_csv("Gel_proteinGroups_HMW_LMW_combined.csv")
...
library(edgeR)
groups = c("Reactive", "Reactive", "Reactive", "Reactive", "Reactive",
           "Control", "Control", "Control", "Control", "Control") %>%
  factor(levels = c("Reactive", "Control")
  )
design = model.matrix(~0+groups)
contrasts = makeContrasts(
  H = groupsReactive - groupsControl,
  levels = design
)
LFQ_edgeR <- DGEList(
...
make_pca_plot <- function(tp, tp_edger, ellipse = T) {
  tp %>%
    ...
    ggbiplot(labels = c("Reactive1", "Reactive2", "Reactive3", "Reactive4", "Reactive5",
