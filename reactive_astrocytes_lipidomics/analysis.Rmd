---
title: "Astrocyte BioMarker Analysis"
author: "Jonathan Fine and Priya Prakash"
date: "2/2/2020"
...
make_heatmap_jf_e1 <- function(tp, design_mat, gr, contrasts, title = "Heat-map", FDR = 0.10) {
  ...
  tp %>%
    ...
    select(-Transition, -type) %>%
    rowwise() %>%
    mutate(mean = mean(c(C1, C2, C3, C4, C5,
                         R1, R2, R3, R4, R5))) %>%
    mutate_if(is.numeric, log2) %>%
...
write_summary_and_results_combined_pp(cm_e1_tbl, mm_e1_tbl,
                                      cells_metab_expr_1, media_metab_expr_1) %>%
  write_csv("results/cm_w_mm_e1.csv")

# write_summary_and_results_combined_pp(cl_e2_tbl, ml_e2_tbl,
#                                       cells_lipid_expr_2, media_lipid_expr_2) %>%
#   write_csv("results/cl_w_ml_e2.csv")
# 
# write_summary_and_results_combined_pp(cm_e2_tbl, mm_e2_tbl,
#                                       cells_metab_expr_2, media_metab_expr_2) %>%
#   write_csv("results/cm_w_mm_e2.csv")
```
