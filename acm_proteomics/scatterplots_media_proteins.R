...
df <- read.csv(file = '4x_full_for_analysis.csv')
df[, 8:12][df[, 8:12] == 0] <- 1


df_sig_p <- filter(df, df$FDR<0.1)
# top100 <- filter(df, df$FDR<8.8e-19)
...
## Normal scale
df %>% 
  ggplot(aes(x=AverageC_lfq,y=AverageR_lfq, color=Fold_change)) + 
  geom_point(alpha=1, color="#595855", size = 3) +
  
  geom_point(data=df_sig_p, 
             aes(x=AverageC_lfq,y=AverageR_lfq),
...
ggtitle("Astrocyte Media Proteins") +
  theme(plot.title = element_text(hjust = 0.5))+
  ggrepel::geom_text_repel(data = df_sig_p, aes(label = Gene.names))

##Log10 scale
mid<-0

df %>% 
  ggplot(aes(x=log10(AverageMS_C),y=log10(AverageMS_R), color=logFC)) + 
  
  geom_point(data = high_logfc, alpha=1, color="red", size = 1.5) +
  
  geom_point(data=mid_logfc,aes(x=log10(AverageMS_C),y=log10(AverageMS_R)),size=1.5)+
  ...
  # ggrepel::geom_text_repel(data = df, aes(label = Gene.names))
